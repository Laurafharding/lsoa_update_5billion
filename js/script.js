...
//╱╱╱╱╱╱╱╱╰╯╱╰╯╱╱╱╱╱╰━━╯
  ...
  function ready (error, topoLSOA, topoLAD, boundsData, config){
    ...
    map = L.map('map',{maxZoom:18,minZoom:12}),
  ...
  map.setView(eval(dvc.config.ons.centre), dvc.config.ons.zoom).addLayer(layerx);
	...
	function handleLayerLAD(layer){
		x = layer.feature.properties.lad17cd;
        layer.setStyle({
		  fillColor: '#fff',
          fillOpacity: 0,
          color:'#fff',
...
function highlightArea(){
	d3.select('#selected').remove();
		...
		d3.select("#areainfo").html(function(d,i){if (!isNaN(rateById[currclass]))  {return "<span>" + dvc.unittext + "</span>" + dvc.prefix + numberFormat(rateById[currclass])} else {return "Data unavailable"}});
...
function addRemoveAreas(oldDistricts, newDistricts) {
	...
	areasToAdd.forEach(function(d,i) {
		...
		d3.json('https://raw.githubusercontent.com/Laurafharding/lsoa-by-lad-topojson/master/geojson/lsoa_lookup_LAD17CD_'+d+'.geojson', function(error, distData) {
			//console.log('https://cdn.ons.gov.uk/assets/topojson/msoa_by_lad/topo_' + d + '.json');

			d3.csv('https://raw.githubusercontent.com/Laurafharding/lsoa-by-lad-topojson/master/Cut3/' + d + '.csv', function(error, csvData) {
	...
	function buildDataInView(d,csvData) {
			...
			rateById = {};
			areaById = {};
			flattenedData.forEach(function(d) {rateById[d.LSOA_code] = +eval("d.y1"); areaById[d.LSOA_code] = d.LSOA_name});
	}
